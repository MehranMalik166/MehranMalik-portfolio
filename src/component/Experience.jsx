  ...
  const experience = [
    {
      ...
      current: false,
      // isCompleted: true,
      description: "Worked on web development projects with a strong focus on responsive and user-friendly frontend development. Built and improved web pages, UI components, layouts, and interactive features while gaining hands-on experience with PHP, MySQL, and website management.",
    ...
    <div id="experience" className="w-full px-[12%] py-10 scroll-mt-20">
      ...
      >
        {experience.map((exp) => (
          <div
            key={exp.id}
            className="relative border border-gray-200 dark:border-white/20 rounded-2xl p-5
          ...
          >
            {exp.current ? (
              <span className="absolute right-5 flex items-center gap-1.5 bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/30 text-[10px] font-semibold px-2.5 py-0.5 rounded-full">
              ...
              <span className="absolute right-5 flex items-center gap-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/30 text-[10px] font-semibold px-2.5 py-0.5 rounded-full">
    <span className="flex items-center justify-center h-3 w-3 rounded-full bg-blue-500/20">
      <svg
        className="h-2 w-2"
        viewBox="0 0 12 12"
        fill="none"
