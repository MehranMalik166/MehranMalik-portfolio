...
function About() {
  ...
  return (
    <div
      id="about"
      className="w-full px-[12%] py-10 mt-10 sm:mt-5 scroll-mt-20 flex justify-center"
    >
        ...
        <div className="flex w-full item justify-center flex-col lg:flex-row items-center gap-20 my-20">
          <div id="left" className="max-w-max mx-auto relative">
            <img
              src={aboutImg}
              alt=""
              className="w-64 h-full sm:w-80 rounded-3xl max-w-none"
            />
            <div className="bg-white w-1/2 aspect-square absolute right-0 bottom-0 rounded-full translate-x-1/4 translate-y-1/3 shadow-[0_4px_55px_rgba(149,0,162,0.15)] flex items-center justify-center">
              <img
                src="https://eliana.prebuiltui.com/assets/circular-text.png"
                alt=""
                className="w-full animate-spin_slow"
              />
              <img
                src="https://eliana.prebuiltui.com/assets/dev-icon.png"
                alt=""
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl">
              {data.map((data) => {
                return (
                  <li
                    key={data.name}
                    className="border border-gray-300 dark:border-white/30 rounded-lg p-6 cursor-pointer hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black dark:hover:shadow-white/80 dark:hover:bg-darkHover/50 "
                  >
                    <img
                      src={data.icon1}
                      alt=""
